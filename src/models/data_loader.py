...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            mask_src = 1-src == 0 # change to ~src == 0 if using pytorch 1.2 or older
            mask_tgt = 1-tgt == 0 # change to ~tgt == 0 if using pytorch 1.2 or older


            clss = torch.tensor(self._pad(pre_clss, -1))
            src_sent_labels = torch.tensor(self._pad(pre_src_sent_labels, 0))
            mask_cls = 1-clss == -1 # change to ~clss == -1 if using pytorch 1.2 or older
            clss[clss == -1] = 0
